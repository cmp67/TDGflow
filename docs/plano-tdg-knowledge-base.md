...
- **Como os selos de perfil/vibe dos fornecedores são gerados** (pedido da Carla, 02/08) — explicar que "Família", "Ultra Luxury", "Boutique" etc. vêm de classificação por IA (Sonnet definiu o vocabulário a partir do conteúdo real das reviews, Haiku classificou cada fornecedor por nome+país+sinais de review), não é dado verificado/objetivo (sem preço médio, sem categoria de estrelas na base) — é reconhecimento de padrão, pode ter caso questionável, e a pessoa pode reportar erro pra correção pontual. Ver `docs/whatsapp-extraction/fase8b_profiles_e_correcoes.md`.
...
4. **4a ✅ feito** (`fase4a_insercao_hoteis_reviews.md`): 685 hotéis + 1.012 reviews inseridos, extração de IA em todas (decisão #13), `import_approval='pending'`. Ponto em aberto: selo "Testado" hoje não olha `import_approval` — hotéis importados já aparecem testados antes da aprovação (decidir antes da Fase 6). **4b ✅ feito** (`fase4b_insercao_contatos.md`): 589 contatos inseridos (114 vinculados a hotel, 475 fornecedor por texto livre), 2 bugs achados na verificação visual e corrigidos direto na base (categoria vazia caindo no fallback "médico"; "Lu" duplicada por dedup pela string bruta em vez do hotel resolvido) — nenhum código de app tocado, nenhum redeploy necessário.
5. **✅ feito** (`fase5_conhecimento_destino.md`): 626 registros inseridos (315 fact + 289 note + 22 link), `import_approval='pending'`. Achado real de schema no caminho — `CHECK (country IS NOT NULL OR hotel_id IS NOT NULL)` travava 32% do conteúdo (conhecimento geral do setor, sem destino central); resolvido com a Carla removendo o `CHECK` e adicionando classificação por `tags text[]` (pandemia/aéreo/golpe + 19 outras, vocabulário fixo). `GET /api/knowledge-tips` corrigido pra ler `tdg_destination_knowledge` (lia a tabela errada, `tdg_knowledge`, sempre vazia) e busca por país/tag.
6. **✅ feito** (`fase6_fila_confirmacao.md`): fila do autor (3ª seção condicional, decisão #14-15) + fila do admin/fallback (decisão #16) construídas juntas. Achado antes de codar: nenhum dos 228 autores reais bate com as 5 contas de `tdg_users` hoje — fila do admin cobre 100% dos 1.638 itens pendentes agora, fila do autor fica pronta e dormente. Desvios avisados: editor de campos simples em vez do wizard de review (não existe modo edição nele), card genérico em vez de `HotelCard` (fila cobre 2 tipos de conteúdo), regra de 30 dias não implementada (não há caso real ainda pra ela valer). Incluiu também, fora do escopo original: editar/excluir contato aberto a qualquer TD.
7. **✅ feito** (`fase7_badges_gamificacao.md`): motor de badges construído — cron diário (`/api/cron/recompute-badges`, primeiro cron do projeto), `GET /api/badges` + "Minhas conquistas" em Meu Perfil, sino com `badge_earned`/`badge_changed`. Dormente hoje por 2 motivos que se autocorrigem com uso real (não backfill): nenhuma review importada tem `agent_id` (mesma parede da Fase 6) + país/categoria não populados nas reviews importadas. Não implementado: badge pill junto ao nome do autor (decisão #19, zero efeito visual sem nenhum badge ainda) e disparo de Pioneira por evento (roda no cron diário em vez de hook por ponto de entrada).
8. **✅ feito** (`fase8_busca_cruzada.md`): `GET /api/search` cruza conhecimento + hotéis + reviews (`status='published'`) + contatos, agrupado por tipo, `import_approval` propagado nos grupos que têm a coluna. `knowledge-tips` voltou a ser só listagem sem busca. Achado no meio do caminho: mais 3 hotéis duplicados (2 mesclados, 1 aguardando confirmação da Carla) e um problema separado — pills de filtro do catálogo (`region`/`profiles`) só funcionam pros 5 hotéis curados, 652 importados sem `profiles` e com lista de região hardcoded — reportado à parte, não corrigido nesta fase.
