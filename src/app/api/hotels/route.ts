...
export async function GET() {
  ...
  const { rows } = await sql`
    SELECT h.id, h.name, h.entity_type, h.location, h.country, h.region, h.description,
           ...
           h.agency_id, (h.agency_id IS NOT NULL) AS is_private,
...
export async function POST(req: NextRequest) {
  ...
  // created_by segue a mesma regra de posse de PATCH/reviews (14/08) — sem
  // isso, um fornecedor privado nasceria "sem dono" e só admin conseguiria
  // editá-lo depois, mesmo tendo sido a própria agência quem cadastrou.
  const { rows } = await sql`
    INSERT INTO tdg_hotels (name, entity_type, location, country, description, agency_id, created_by)
    VALUES (${name}, ${entityType}, ${location}, ${country}, ${description}, ${agencyId}, ${session.user.email})
    ON CONFLICT (lower(trim(name)), entity_type, agency_id) WHERE agency_id IS NOT NULL DO NOTHING
    RETURNING id, name, entity_type, location, country, description, agency_id, created_by
  `
  if (!rows[0]) {
    // Corrida: outra requisição criou o mesmo nome pra esta agência entre o SELECT e o INSERT.
    const { rows: raced } = await sql`
      SELECT id, name, entity_type, location, country, description, agency_id, created_by FROM tdg_hotels
