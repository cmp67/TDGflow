...
export default function SubscriptionConfirmation() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'var(--bg)' }}>
      <div className="w-full max-w-sm text-center" style={{ background: 'var(--surface)', border: '1px solid var(--border-subtle)', borderRadius: 16, padding: '32px 24px' }}>

        {stillChecking && (
          <>
            <Loader2 className="animate-spin mx-auto mb-4" size={32} style={{ color: 'var(--gold)' }} />
        ...
        {(result.status === 'pending' || result.status === 'none') && gaveUp && (
          <>
            <Clock className="mx-auto mb-4" size={32} style={{ color: 'var(--accent-warm)' }} />
            <h1 className="text-lg font-semibold mb-2" style={{ color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>
              Ainda processando
            </h1>
            ...
            <button
              onClick={retryNow}
              style={{ padding: '9px 16px', borderRadius: 9, border: 'none', background: 'var(--gold)', color: 'var(--surface)', fontSize: '0.8125rem', fontWeight: 600, cursor: 'pointer' }}
            >
              Verificar novamente
            </button>
          </>
        ...
        {result.status === 'authorized' && (
          <>
            <CheckCircle2 className="mx-auto mb-4" size={32} style={{ color: 'var(--success)' }} />
            <h1 className="text-lg font-semibold mb-2" style={{ color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>
            ...
            {fmtAmount(result.transactionAmount) && (
              <p className="text-sm mb-1" style={{ color: 'var(--text-muted)' }}>
                Cobrança mensal: <strong style={{ color: 'var(--text-primary)' }}>{fmtAmount(result.transactionAmount)}</strong>
              </p>
            )}
            {fmtDate(result.nextPaymentDate) && (
              <p className="text-sm mb-4" style={{ color: 'var(--text-muted)' }}>
                Próxima cobrança: <strong style={{ color: 'var(--text-primary)' }}>{fmtDate(result.nextPaymentDate)}</strong>
              </p>
            )}
            <Link href="/flow/billing" style={{ display: 'inline-block', marginTop: 8, padding: '9px 16px', borderRadius: 9, background: 'var(--gold)', color: 'var(--surface)', fontSize: '0.8125rem', fontWeight: 600, textDecoration: 'none' }}>
        ...
        {(result.status === 'rejected' || result.status === 'cancelled') && (
          <>
            <XCircle className="mx-auto mb-4" size={32} style={{ color: 'var(--error)' }} />
            <h1 className="text-lg font-semibold mb-2" style={{ color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>
            ...
            <Link href="/flow/billing" style={{ display: 'inline-block', padding: '9px 16px', borderRadius: 9, background: 'var(--gold)', color: 'var(--surface)', fontSize: '0.8125rem', fontWeight: 600, textDecoration: 'none' }}>
              Tentar novamente
            </Link>
          </>
        )}
