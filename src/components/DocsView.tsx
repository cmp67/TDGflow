...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; border: string }> = {
  'visa-free':       { label: 'Sem visto',       color: '#2E7D4F', bg: 'rgba(46,125,79,.08)',   border: 'rgba(46,125,79,.22)' },
  'visa-on-arrival': { label: 'Visto na chegada', color: '#B6410F', bg: 'rgba(182,65,15,.08)',  border: 'rgba(182,65,15,.22)' },
  'e-visa':          { label: 'e-Visa',           color: '#008C94', bg: 'rgba(0,140,148,.08)',  border: 'rgba(0,140,148,.22)' },
  'eta':             { label: 'ETA',              color: '#6B21A8', bg: 'rgba(107,33,168,.08)', border: 'rgba(107,33,168,.22)' },
  'visa-required':   { label: 'Visto obrigatório', color: '#C0392B', bg: 'rgba(192,57,43,.08)', border: 'rgba(192,57,43,.22)' },
  'no-entry':        { label: 'Entrada proibida', color: '#7F1D1D', bg: 'rgba(127,29,29,.08)',  border: 'rgba(127,29,29,.22)' },
  'unknown':         { label: 'Consultar',        color: '#4A7580', bg: 'rgba(74,117,128,.08)', border: 'rgba(74,117,128,.22)' },
}
...
function SecurityIcon({ level }: { level: number }) {
  if (level === 1) return <ShieldCheck size={14} style={{ color: '#2E7D4F' }} />
  if (level === 2) return <ShieldAlert size={14} style={{ color: '#B6410F' }} />
  if (level === 3) return <ShieldX size={14} style={{ color: '#C0392B' }} />
...
function SecurityBadge({ level, label }: { level: number; label: string }) {
  const colors: Record<number, { color: string; bg: string }> = {
    1: { color: '#2E7D4F', bg: 'rgba(46,125,79,.08)' },
    2: { color: '#B6410F', bg: 'rgba(182,65,15,.08)' },
    3: { color: '#C0392B', bg: 'rgba(192,57,43,.08)' },
...
function ResultCard({ result, onClose }: { result: TravelRequirements; onClose: () => void }) {
  ...
  return (
    ...
    >
      <button
        onClick={onClose}
        style={{ position: 'absolute', top: 12, right: 12, background: 'none', border: 'none', cursor: 'pointer', color: '#4A7580', padding: 4 }}
      >
        <X size={14} />
      </button>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <h2 style={{ fontSize: '1.25rem', fontWeight: 600, color: '#112630', letterSpacing: '-0.02em' }}>
          {result.destination}
        </h2>
        {result.iso2 && result.iso2 !== 'XX' && (
          <p style={{ fontSize: '0.75rem', color: '#4A7580', marginTop: 2 }}>Passaporte brasileiro · {result.iso2}</p>
        )}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
        ...
        <div style={{ padding: '12px 14px', borderRadius: 10, background: visCfg.bg, border: `1px solid ${visCfg.border}` }}>
          <p style={{ fontSize: '0.6rem', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#4A7580', marginBottom: 6 }}>Visto</p>
          <VisaBadge status={result.visa_status} />
          {result.visa_details && (
            <p style={{ fontSize: '0.7rem', color: '#104C64', marginTop: 6, lineHeight: 1.4 }}>{result.visa_details}</p>
          )}
        </div>

        {/* Security */}
        <div style={{ padding: '12px 14px', borderRadius: 10, background: 'var(--surface-high)', border: '1px solid var(--border)' }}>
          <p style={{ fontSize: '0.6rem', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#4A7580', marginBottom: 6 }}>Segurança</p>
          {result.security_level ? (
            <>
              <SecurityBadge level={result.security_level} label={result.security_label ?? ''} />
              {result.security_message && (
                <p style={{ fontSize: '0.7rem', color: '#104C64', marginTop: 6, lineHeight: 1.4 }}>{result.security_message}</p>
              )}
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 6 }}>
                <span style={{ fontSize: '0.5625rem', color: '#7BA8B2', background: '#F0F5F7', borderRadius: 4, padding: '1px 6px', fontWeight: 500 }}>
                  US State Dept
                </span>
                {result.security_updated_at && (
                  <>
                    <span style={{ fontSize: '0.5625rem', color: '#B8CDD2' }}>·</span>
                    <span style={{ fontSize: '0.5625rem', color: '#7BA8B2' }}>
            ...
            <span style={{ fontSize: '0.7rem', color: '#4A7580' }}>Sem dados</span>
          )}
        </div>
      </div>
      ...
      {result.civp_required && (
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 14px', borderRadius: 10, background: 'rgba(182,65,15,.06)', border: '1px solid rgba(182,65,15,.2)', marginBottom: 10 }}>
          <Syringe size={14} style={{ color: '#B6410F', flexShrink: 0, marginTop: 1 }} />
          <div>
            <p style={{ fontSize: '0.75rem', fontWeight: 600, color: '#B6410F', marginBottom: 2 }}>CIVP obrigatório</p>
            <p style={{ fontSize: '0.7rem', color: '#104C64', lineHeight: 1.4 }}>Certificado Internacional de Vacinação contra febre amarela exigido para viajantes partindo do Brasil.</p>
          </div>
        </div>
      )}
      ...
      {result.eta_required && (
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 14px', borderRadius: 10, background: 'rgba(107,33,168,.05)', border: '1px solid rgba(107,33,168,.2)', marginBottom: 10 }}>
          ...
          <p style={{ fontSize: '0.7rem', color: '#104C64', lineHeight: 1.4 }}>Requer ETA (Autorização Eletrônica de Viagem) antes do embarque.</p>
        </div>
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginTop: 4 }}>
          {result.notes.map((n, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 7 }}>
              <ChevronRight size={11} style={{ color: '#008C94', flexShrink: 0, marginTop: 2 }} />
              <p style={{ fontSize: '0.75rem', color: '#104C64', lineHeight: 1.4 }}>{n}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {result.source?.length > 0 && (
        <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid #D0E2E5' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 6 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <span style={{ fontSize: '0.6rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#4A7580' }}>Fontes</span>
              {result.source.map((s, i) => {
                ...
                return src.url ? (
                  <a key={i} href={src.url} target="_blank" rel="noopener noreferrer"
                    style={{ fontSize: '0.65rem', fontWeight: 500, color: '#008C94', textDecoration: 'underline', textDecorationStyle: 'dotted', textUnderlineOffset: 2 }}>
                    {src.label}
                  </a>
                ) : (
                  <span key={i} style={{ fontSize: '0.65rem', fontWeight: 500, color: '#4A7580' }}>{src.label}</span>
                )
              })}
            </div>
            {result.fetched_at && (
              <span style={{ fontSize: '0.6rem', color: '#B8D0D5', whiteSpace: 'nowrap' }}>
    ...
    <div className="flex-1 overflow-y-auto p-4 md:p-6" style={{ maxWidth: 680, margin: '0 auto', width: '100%' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: '0.8125rem', color: '#4A7580', marginTop: 4 }}>
          ...
          <strong style={{ color: '#008C94' }}>
            {PASSPORTS.find(p => p.code === passport)?.flag}{' '}
            {PASSPORTS.find(p => p.code === passport)?.label}
          </strong>
        </p>
        {/* Data sources note */}
        <div style={{ marginTop: 10, display: 'flex', alignItems: 'flex-start', gap: 7, padding: '8px 12px', background: '#F0F7F8', borderRadius: 8, border: '1px solid #D0E2E5' }}>
          <span style={{ fontSize: '0.65rem', marginTop: 1 }}>🔍</span>
          <p style={{ fontSize: '0.6875rem', color: '#4A7580', lineHeight: 1.5, margin: 0 }}>
            Dados consultados em tempo real via{' '}
            <a href="https://rapidapi.com/TravelBuddyAI/api/visa-requirement" target="_blank" rel="noopener noreferrer" style={{ color: '#008C94', textDecoration: 'underline', textDecorationStyle: 'dotted' }}>Travel Buddy AI</a>
            {' '}· com dados complementares de{' '}
            <a href="https://www.who.int/ith/en/" target="_blank" rel="noopener noreferrer" style={{ color: '#008C94', textDecoration: 'underline', textDecorationStyle: 'dotted' }}>WHO/ANVISA</a>
            {' '}e{' '}
            <a href="https://travel.state.gov/content/travel/en/international-travel.html" target="_blank" rel="noopener noreferrer" style={{ color: '#008C94', textDecoration: 'underline', textDecorationStyle: 'dotted' }}>US State Dept</a>
      ...
      <div style={{ marginBottom: 14 }}>
        <p style={{ fontSize: '0.6rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#4A7580', marginBottom: 7 }}>
          Passaporte
        </p>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: active ? '#E6F4F5' : '#FFFFFF',
                  border: active ? '1.5px solid #008C94' : '1px solid #D0E2E5',
                  fontSize: '0.75rem', fontWeight: active ? 600 : 400,
                  color: active ? '#008C94' : '#104C64',
      ...
      <div style={{ position: 'relative', marginBottom: 16 }}>
        <Search size={15} style={{ position: 'absolute', left: 13, top: '50%', transform: 'translateY(-50%)', color: '#4A7580', pointerEvents: 'none' }} />
        <input
          ref={inputRef}
          className="input"
        ...
        />
        {loading && (
          <Loader2 size={14} className="animate-spin" style={{ position: 'absolute', right: 13, top: '50%', transform: 'translateY(-50%)', color: '#008C94' }} />
        )}
      </div>
        ...
        {QUICK_DESTINATIONS.map(d => (
          <button
            ...
            style={{
              display: 'inline-flex', alignItems: 'center', gap: 5,
              padding: '5px 11px', borderRadius: 999,
              background: '#FFFFFF', border: '1px solid #D0E2E5',
              fontSize: '0.75rem', color: '#104C64', cursor: 'pointer',
              transition: 'all 150ms', fontWeight: 500,
            }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#008C94'; (e.currentTarget as HTMLElement).style.color = '#008C94' }}
            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = '#D0E2E5'; (e.currentTarget as HTMLElement).style.color = '#104C64' }}
          >
      ...
      {error && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', borderRadius: 10, background: 'rgba(192,57,43,.06)', border: '1px solid rgba(192,57,43,.2)', marginBottom: 16 }}>
          ...
          <p style={{ fontSize: '0.8125rem', color: '#C0392B' }}>Destino não encontrado. Tente outro nome ou use o Modo Flow para perguntas detalhadas.</p>
        </div>
      )}
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid #D0E2E5', marginBottom: 16 }}>
        {TABS.map(t => (
          <button
            key={t.key}
            ...
            style={{
              ...
              color: tab === t.key ? '#008C94' : '#4A7580',
              borderBottom: tab === t.key ? '2px solid #008C94' : '2px solid transparent',
            ...
            {['all', 'visa-free', 'visa-on-arrival', 'e-visa', 'eta', 'visa-required'].map(f => (
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer', transition: 'all 150ms',
                  background: visaFilter === f ? '#008C94' : '#FFFFFF',
                  color: visaFilter === f ? '#FFFFFF' : '#104C64',
                  border: visaFilter === f ? '1px solid #008C94' : '1px solid #D0E2E5',
                }}
              >
            ...
            {filteredVisa.map((row, i) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: 8, background: i % 2 === 0 ? '#FFFFFF' : '#F3F7F8',
                  border: '1px solid transparent',
                  cursor: 'pointer', transition: 'all 150ms',
                }}
                onClick={() => { setQuery(row.destination); search(row.destination) }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#D0E2E5' }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'transparent' }}
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: '0.8125rem', fontWeight: 500, color: '#112630' }}>{row.destination}</p>
                  <p style={{ fontSize: '0.7rem', color: '#4A7580', marginTop: 1 }}>{row.detail}</p>
                </div>
                <VisaBadge status={row.status} />
              </div>
            ))}
          </div>
          <p style={{ fontSize: '0.6875rem', color: '#4A7580', marginTop: 12, textAlign: 'center' }}>
      ...
      {tab === 'saude' && (
        <div>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '12px 14px', borderRadius: 10, background: 'rgba(182,65,15,.06)', border: '1px solid rgba(182,65,15,.2)', marginBottom: 16 }}>
            <Syringe size={14} style={{ color: '#B6410F', flexShrink: 0, marginTop: 1 }} />
            <p style={{ fontSize: '0.75rem', color: '#104C64', lineHeight: 1.5 }}>
              <strong style={{ color: '#B6410F' }}>CIVP</strong> — Certificado Internacional de Vacinação contra Febre Amarela. Viajantes partindo do Brasil podem ser obrigados a apresentar o documento na chegada ao destino.
            </p>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            {HEALTH_TABLE.map((row, i) => (
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: '0.8125rem', fontWeight: 500, color: '#112630' }}>{row.destination}</p>
                  <p style={{ fontSize: '0.7rem', color: '#4A7580', marginTop: 1 }}>{row.notes}</p>
                </div>
                {row.civp ? (
                  <span style={{ fontSize: '0.65rem', fontWeight: 600, padding: '3px 8px', borderRadius: 999, color: '#B6410F', background: 'rgba(182,65,15,.08)', border: '1px solid rgba(182,65,15,.2)', whiteSpace: 'nowrap' }}>
                    CIVP obrigatório
                  </span>
                ) : (
                  <span style={{ fontSize: '0.65rem', fontWeight: 500, padding: '3px 8px', borderRadius: 999, color: '#4A7580', background: '#F3F7F8', border: '1px solid #D0E2E5', whiteSpace: 'nowrap' }}>
                    CIVP opcional
                  </span>
                )}
              </div>
      ...
      {tab === 'seguranca' && (
        <div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
            {[
              { level: 1, label: 'Nível 1 — Normal',       color: '#2E7D4F' },
              { level: 2, label: 'Nível 2 — Atenção',      color: '#B6410F' },
              { level: 3, label: 'Nível 3 — Reconsiderar', color: '#C0392B' },
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            {SECURITY_TABLE.sort((a, b) => a.level - b.level).map((row, i) => (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: '0.8125rem', fontWeight: 500, color: '#112630', margin: 0 }}>{row.destination}</p>
                  <p style={{ fontSize: '0.6875rem', color: '#4A7580', marginTop: 3, lineHeight: 1.45, margin: '3px 0 0' }}>{row.detail}</p>
      ...
      {tab === 'materiais' && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 24px', textAlign: 'center', gap: 12 }}>
          <div style={{ width: 48, height: 48, borderRadius: 14, background: '#E8F0F2', border: '1px solid #D0E2E5', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <FileText size={20} style={{ color: '#4A7580' }} />
          </div>
          <p style={{ fontSize: '0.875rem', fontWeight: 500, color: '#112630' }}>Materiais em breve</p>
          <p style={{ fontSize: '0.8rem', color: '#4A7580', maxWidth: 320, lineHeight: 1.5 }}>
