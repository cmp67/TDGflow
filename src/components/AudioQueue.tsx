  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ background: 'var(--tdgflow-surface)', border: '1px solid var(--tdgflow-border)', maxHeight: '85vh' }}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 flex-shrink-0" style={{ borderBottom: '1px solid var(--tdgflow-border)' }}>
          <div className="flex items-center gap-3">
            <h2 className="text-base font-semibold" style={{ color: 'var(--tdgflow-text-primary)' }}>Fila de Áudios</h2>
            {pendingCount > 0 && (
        ...
        <div className="overflow-y-auto flex-1 p-4 space-y-2">
          {loading && <p className="text-center text-sm py-8" style={{ color: 'var(--tdgflow-text-muted)' }}>Carregando...</p>}
          {!loading && items.length === 0 && (
            <p className="text-center text-sm py-8" style={{ color: 'var(--tdgflow-text-muted)' }}>Nenhum áudio registrado ainda.</p>
          )}
            ...
            {items.map(item => {
              ...
              return (
                <motion.div
                  ...
                  style={{ border: '1px solid var(--tdgflow-border)' }}
                >
                  {/* Row */}
                  <div className="flex items-center gap-3 p-4">
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: 'var(--tdgflow-surface-high)' }}>
                      <Mic size={16} style={{ color: 'var(--tdgflow-text-muted)' }} />
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <>
                          <div className="flex items-center gap-2 flex-wrap">
                            {item.interlocutor_name
                              ? <span className="text-sm font-medium" style={{ color: 'var(--tdgflow-text-primary)' }}>{item.interlocutor_name}</span>
                              : <span className="text-sm italic" style={{ color: 'var(--tdgflow-text-muted)' }}>Sem interlocutor</span>
                            }
                            {item.interlocutor_company && (
                              <span className="text-xs" style={{ color: 'var(--tdgflow-text-muted)' }}>· {item.interlocutor_company}</span>
                            )}
                          </div>
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className="text-xs" style={{ color: 'var(--tdgflow-text-muted)' }}>{item.agent_name}</span>
                            <span style={{ color: 'var(--tdgflow-border-light)' }}>·</span>
                            <span className="text-xs" style={{ color: 'var(--tdgflow-text-muted)' }}>{formatDate(item.created_at)}</span>
                          </div>
                        </>
                      )}
                      ...
                      {item.transcribe_error && (
                        <div className="flex items-center gap-1.5 mt-1.5">
                          <AlertCircle size={11} style={{ color: 'var(--tdgflow-error)', flexShrink: 0 }} />
                          <span style={{ fontSize: '0.6875rem', color: 'var(--tdgflow-error)' }}>{item.transcribe_error}</span>
                        </div>
                      )}
                    </div>
                      ...
                      {isEditingThis ? (
                        ...
                        <>
                          ...
                          <button
                            ...
                            title="Editar"
                          >
                            <Pencil size={13} style={{ color: 'var(--tdgflow-text-muted)' }} />
                          </button>
                          ...
                          {(item.status === 'transcribed' || item.status === 'confirmed') && (
                            <button
                              ...
                              className="btn-ghost"
                              style={{ padding: '4px 10px', fontSize: '0.75rem', color: 'var(--tdgflow-navy)' }}
                            >
                  ...
                  <AnimatePresence>
                    {isExpanded && item.summary && (
                      ...
                      >
                        <div className="p-4 space-y-2" style={{ borderTop: '1px solid var(--tdgflow-border)', background: 'var(--tdgflow-surface-high)' }}>
                          {item.summary.hotel_name && (
                            <p className="text-sm" style={{ color: 'var(--tdgflow-text-secondary)' }}>
                              <span className="font-medium" style={{ color: 'var(--tdgflow-text-primary)' }}>Hotel:</span>{' '}{item.summary.hotel_name}
                            </p>
                          )}
                          {item.summary.commission_rate && (
                            <p className="text-sm" style={{ color: 'var(--tdgflow-text-secondary)' }}>
                              <span className="font-medium" style={{ color: 'var(--tdgflow-text-primary)' }}>Comissão:</span>{' '}
                              <span style={{ color: 'var(--tdgflow-navy)' }}>{item.summary.commission_rate}%</span>
                            </p>
                          )}
                          {Array.isArray(item.summary.highlights) && item.summary.highlights.length > 0 && (
                            <div>
                              <p className="text-xs font-medium uppercase tracking-wider mb-1.5" style={{ color: 'var(--tdgflow-text-muted)' }}>Destaques</p>
                              {item.summary.highlights.map((h, i) => (
                                <p key={i} className="text-sm" style={{ color: 'var(--tdgflow-text-secondary)' }}>· {h}</p>
                              ))}
                            </div>
                          )}
                          {item.summary.notes && (
                            <p className="text-sm italic" style={{ color: 'var(--tdgflow-text-muted)' }}>{item.summary.notes}</p>
                          )}
                          {item.transcript && (
                            <details className="mt-2">
                              <summary className="text-xs cursor-pointer" style={{ color: 'var(--tdgflow-text-muted)' }}>Ver transcrição completa</summary>
                              <p className="text-xs mt-2 leading-relaxed" style={{ color: 'var(--tdgflow-text-muted)' }}>{item.transcript}</p>
                            </details>
                          )}
                        </div>
                      </motion.div>
