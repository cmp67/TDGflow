...
function ContactCard({ contact, copiedId, onCopy, highlightId, onChanged }: {
  contact: NetworkContact
  ...
  // Confirmação de exclusão expira sozinha — evita clique acidental duplo
  // virar exclusão sem querer, sem precisar de modal.
  useEffect(() => {
  ...
  async function handleDelete(e: React.MouseEvent) {
    ...
    setDeleting(true)
    await fetch(`/api/hotel-contacts?id=${contact.id}`, { method: 'DELETE' })
    onChanged()
  }
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 8 }}>
        ...
        <button
          ...
          style={{ flexShrink: 0, background: 'none', border: 'none', cursor: 'pointer', padding: 4, color: 'var(--tdgflow-text-muted)', display: 'flex' }}
        >
          <Pencil size={13} />
        </button>
        <button
          onClick={handleDelete}
          title={confirmingDelete ? 'Clique de novo pra confirmar' : 'Excluir contato'}
          disabled={deleting}
          style={{
            flexShrink: 0, background: confirmingDelete ? 'var(--tdgflow-error)' : 'none',
            border: confirmingDelete ? 'none' : 'none', borderRadius: 6,
            cursor: 'pointer', padding: 4,
            color: confirmingDelete ? '#fff' : 'var(--tdgflow-text-muted)',
            display: 'flex', alignItems: 'center', gap: 3,
            fontSize: '0.625rem', transition: 'all 120ms',
          }}
        >
          {deleting ? <Loader2 size={13} className="animate-spin" /> : <Trash2 size={13} />}
          {confirmingDelete && !deleting && 'confirmar?'}
        </button>
        {contact.whatsapp && (
          <button
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 10 }}>
            <AnimatePresence mode="popLayout">
              {contacts.map(contact => (
                <ContactCard key={contact.id} contact={contact} copiedId={copiedId} onCopy={handleCopy} highlightId={highlightContactId} onChanged={load} />
              ))}
            </AnimatePresence>
          </div>
        )}
